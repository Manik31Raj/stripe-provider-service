...
public class InvoiceGeneratorHelper {
    ...
    @Value("${stripe.invoice-url}")
    private String getInvoiceGeneratorUrl;
    ...
    public HttpRequest prepareHttpRequest(String id) {
        ...
        log.info("Preparing HTTP request to generate invoice for payment with id: {}", id);

        HttpHeaders headers = new HttpHeaders();
        headers.setBasicAuth(stripeAPIKey, Constant.EMPTY_STRING);

        String invoiceGeneratorUrl = getInvoiceGeneratorUrl.replace("{id}", id);
        log.info("InvoiceGenerator URL: {}", invoiceGeneratorUrl );

        HttpRequest httpRequest = new HttpRequest();
        httpRequest.setHttpMethod(HttpMethod.GET);
        httpRequest.setUrl(invoiceGeneratorUrl);
    ...
    public StripeInvoiceGeneratorResponse processResponse(ResponseEntity<String> httpResponse) {
        ...
        if (httpResponse.getStatusCode().is2xxSuccessful()) {
            ...
            StripeInvoiceGeneratorResponse response = jsonUtil.convertJsonToObject(httpResponse.getBody(), StripeInvoiceGeneratorResponse.class);
            log.info("Parsed StripeInvoiceGeneratorResponse : {}", response);

            if(response != null && response.getId() != null) {
                log.info("StripeInvoiceGeneratorResponse is valid and contains required fields");
        ...
        throw new StripeProviderException(
                ErrorCodeEnum.PAYMENT_INCOMPLETE.getErrorCode(),
                ErrorCodeEnum.PAYMENT_INCOMPLETE.getErrorMessage(),
